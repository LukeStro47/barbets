...
export function DemoRevealCard({
  ...
  return (
    <div
      className="animate-demo-fade-up-scale relative overflow-visible rounded-[28px] bg-gradient-to-br from-ink via-ink to-ink text-white shadow-lg shadow-ink/25"
    >
      <div className="px-6 pt-6 pb-[18px]">
        ...
        <p className="mb-1.5 text-[11px] font-bold tracking-[0.1em] text-signal uppercase">Demo &middot; Settled</p>
        <p className="mb-4 max-w-[88%] text-[22px] leading-[1.2] font-extrabold tracking-[-0.01em]">{question}</p>

        <div className="flex items-center gap-3.5">
          <div className="flex h-[68px] w-[68px] shrink-0 -rotate-6 items-center justify-center rounded-full border-[3px] border-ink/20 bg-signal text-center text-lg leading-[1.1] font-extrabold uppercase text-ink shadow-[0_8px_18px_-6px_rgba(232,163,61,0.55)]">
            {headline}
          </div>
          <p className="text-[13px] leading-[1.45] text-white/70">The vote upheld the call.</p>
        </div>
      </div>
      ...
      <div className="px-6 pt-[18px] pb-[22px]">
        <p className="mb-2.5 text-[11px] font-bold tracking-[0.1em] text-signal uppercase">Odds at close</p>
        ...
        <div className="mb-[18px] flex h-2 overflow-hidden rounded-full bg-white/10">
          <div
            className="h-full bg-signal transition-[width] duration-[900ms] ease-[cubic-bezier(0.22,1,0.36,1)] motion-reduce:transition-none"
        ...
        <p className="mb-2.5 text-[11px] font-bold tracking-[0.1em] text-signal uppercase">Who called it</p>
          ...
          {outcome.callers.map((c, i) => (
            <li
              ...
              className={cn(
                ...
                c.isYou && '-mx-2 bg-signal/10 px-2 py-1'
              )}
            ...
            >
              <div className="flex min-w-0 items-center gap-2">
                ...
                <span className={cn('truncate text-[14px] font-extrabold', c.isYou ? 'text-on-ink' : 'text-white')}>
                  {c.isYou ? 'You' : `@${c.nickname}`}
                </span>
                <span className="shrink-0 text-xs text-white/50">{formatTokens(c.amount)} &rarr;</span>
              </div>
              <div className="flex shrink-0 items-baseline gap-1.5">
                <b className="text-[15px] font-extrabold text-on-ink">{formatTokens(c.payout)}</b>
                <span className="text-[10px] font-bold text-white/40">{(c.payout / c.amount).toFixed(1)}&times;</span>
              </div>
            </li>
          ))}
        </ul>
        ...
        <div className="mt-[18px] flex flex-col items-center gap-1.5 border-t border-white/10 pt-3.5">
          ...
          <span className="text-[10px] tracking-[0.02em] text-white/30">This was a demo, no real tokens moved.</span>
        </div>
      </div>
    </div>
  );
