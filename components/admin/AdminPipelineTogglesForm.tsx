...
import { setPipelineEnabled, type PipelineHealth, type PipelineSetting } from '@/lib/actions/admin';
...
// leaves sweep_failures empty (nothing ran to fail), which would otherwise read as healthy.
// Sports' two weekly jobs need a days-scaled threshold, not an hours-scaled one like resolve — a
// job that only ever runs once a week is expected to go quiet for days between runs; roughly 1.5x
// its own 7-day interval gives room for "prepare ran Monday, publish hasn't run yet, it's only
// Tuesday morning" without flapping, while still catching a genuinely missed week.
const STALE_AFTER_MINUTES: Record<PipelineSetting['pipeline'], Partial<Record<PipelineHealth['job'], number>>> = {
...
const STATUS_BADGE: Record<Status, { label: string; className: string }> = {
  off: { label: 'Off', className: 'bg-rule text-muted' },
  failing: { label: 'Failing', className: 'bg-alert-bg text-alert' },
  stale: { label: 'Stale', className: 'bg-signal-tint text-signal-deep' },
  healthy: { label: 'Healthy', className: 'bg-gain-bg text-gain' },
  unknown: { label: 'No runs yet', className: 'bg-rule text-muted' },
};
        ...
        {rows.map((r) => {
          ...
          return (
            <div key={r.pipeline} className="space-y-2 px-3 py-2.5">
              <label className="flex items-center justify-between gap-3">
                <span className="flex flex-col">
                  <span className="flex items-center gap-2">
                    <span className="text-sm font-semibold text-ink">{PIPELINE_LABEL[r.pipeline]}</span>
              ...
              {jobs.length > 0 && (
                <div className="grid grid-cols-2 gap-2 rounded-lg bg-rule/60 px-2.5 py-2 text-xs text-muted">
                  {jobs.map((j) => (
                    <div key={j.job}>
                      <p className="font-semibold text-muted">{JOB_LABEL[j.job]}</p>
                      {j.last_run_at ? (
                        <p>
      ...
      {confirming && (
        <Modal onClose={() => setConfirming(null)}>
          ...
          <p className="text-sm leading-[1.55] text-muted">
            {confirming.next
              ? `This starts real calls to ${PIPELINE_API[confirming.pipeline]} and creates real markets in the public NFL and CFB groups on schedule (${PIPELINE_SCHEDULE[confirming.pipeline].toLowerCase()}).`
