...
export function SeasonRecapHero({
  ...
  return (
    <div className="relative overflow-hidden rounded-[28px] bg-gradient-to-br from-ink via-ink to-ink px-5 pt-[22px] pb-5">
      ...
      <div className="relative">
        ...
        <p className="mt-1.5 font-display text-[27px] leading-[1.1] font-extrabold tracking-[-0.02em] text-white">{seasonName}</p>
        ...
        {viewer === 'owner' ? (
          champion && (
            <div className="mt-5 flex items-center gap-3.5">
              <span className="flex h-[62px] w-[62px] shrink-0 -rotate-6 items-center justify-center rounded-full border-[3px] border-signal bg-ink text-[26px] shadow-[0_8px_18px_-6px_rgba(232,163,61,0.55)]">
                🏆
              </span>
              <span className="min-w-0 flex-1">
                <span className="block text-[10px] font-extrabold tracking-[0.1em] text-on-ink uppercase">Champion</span>
                <Mention
          ...
          <div className="mt-5 flex items-end gap-3.5">
            <span className="min-w-0 flex-1">
              <span className="block text-[10px] font-extrabold tracking-[0.1em] text-on-ink uppercase">You finished</span>
              <span className="mt-1 block font-display text-[44px] leading-none font-extrabold tracking-[-0.03em] text-white">
            ...
            <span className="shrink-0 text-right">
              <span className="block text-xl font-extrabold text-on-ink">{formatTokens(you?.balance ?? 0)}</span>
              <span className="mt-0.5 block text-[10.5px] font-extrabold tracking-[0.07em] text-white/45 uppercase">Final tokens</span>
            </span>
          </div>
        )}
        ...
        {viewer === 'owner' ? (
          <div className="mt-[18px] flex gap-3">
            {[runnerUp, finalBalances[2]].filter((m): m is FinalBalanceRow => !!m).map((m, i) => (
              <span key={m.user_id} className="flex-1 rounded-2xl bg-white/[0.06] px-3 py-2.5">
                <span className="block text-[10px] font-extrabold tracking-[0.08em] text-white/40 uppercase">
                  {i === 0 ? '2nd' : '3rd'}
                  {m.user_id === viewerUserId ? ' · you' : ''}
                </span>
                <Mention nickname={m.nickname} className="mt-[3px] block text-sm font-extrabold text-white" />
                <span className="block text-xs text-on-ink">{formatTokens(m.balance)}</span>
              </span>
            ))}
          </div>
        ) : (
          <div className="mt-[18px] flex gap-3">
            <span className="flex-1">
              <span className="block text-base font-extrabold text-white">{formatSignedTokens(viewerNet ?? 0)}</span>
              <span className="block text-[10px] font-extrabold tracking-[0.07em] text-white/45 uppercase">net for the season</span>
            </span>
            <span className="flex-1">
              <span className="block text-base font-extrabold text-white">{viewerAccuracy == null ? '—' : `${viewerAccuracy}%`}</span>
              <span className="block text-[10px] font-extrabold tracking-[0.07em] text-white/45 uppercase">accuracy</span>
            </span>
            <span className="flex-1">
              <span className="block text-base font-extrabold text-white">{viewerTitlesHeld ?? 0}</span>
