...
export function LostTitleCard({ label, iconKey, toNickname }: { label: string; iconKey: string; toNickname: string }) {
  return (
    <div className="rounded-[20px] border border-dashed border-hairline p-3.5">
      <span className="flex h-11 w-11 items-center justify-center rounded-full border border-dashed border-hairline">
        <AwardGlyph iconKey={iconKey} stroke="var(--color-faint)" size={22} />
      </span>
      <p className="mt-3 text-[10px] font-extrabold tracking-[0.1em] text-faint uppercase">Lost this season</p>
      <p className="mt-0.5 text-[15.5px] leading-[1.15] font-extrabold tracking-[-0.01em] text-muted">{label}</p>
      <p className="mt-1 text-[11.5px] leading-[1.35] text-faint">
