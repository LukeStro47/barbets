  ...
  if (step === 'confirm') {
    return (
      <div className="flex flex-1 flex-col items-center justify-center px-7 py-11 pt-[calc(env(safe-area-inset-top)+2.75rem)] text-center">
        <span className="text-xs font-bold tracking-[2px] text-signal uppercase">You're invited</span>

        <div className="mt-5 w-full rounded-[24px] border border-hairline bg-surface px-5 pt-8 pb-[34px]">
          <GroupAvatar
            name={groupName}
            avatarKey={groupAvatarKey}
            fallbackClassName="bg-rule text-[26px] text-signal"
          ...
          <p className="mt-5 font-display text-[26px]/[34px] font-extrabold tracking-[-0.02em] text-balance text-ink">
            {groupName}
          </p>
        </div>
    ...
    <div className="flex flex-1 flex-col px-7 pb-8 pt-[calc(env(safe-area-inset-top)+3.25rem)]">
      <div className="flex items-center justify-between">
        <button
          ...
          className="-ml-1 inline-flex items-center gap-0.5 text-sm font-semibold text-muted hover:text-ink"
        >
          <CaretLeftIcon className="h-4 w-4" />
          Back
        </button>
        <span className="text-xs font-bold tracking-[1.6px] text-muted uppercase">Step 2 of 2</span>
      </div>

      <h1 className="mt-11 font-display text-[34px]/[38px] font-extrabold tracking-[-0.03em] text-ink">
        Pick your name.
      </h1>
      <p className="mt-2.5 text-base/6 text-muted">
      ...
      <div className="mt-10 flex items-baseline gap-1 border-b-2 border-signal pb-3">
        <span className="font-display text-[32px] font-extrabold text-faint">@</span>
        <input
          value={nickname}
          ...
          autoFocus
          aria-label="Nickname"
          className="w-full min-w-0 bg-transparent font-display text-[32px] font-extrabold text-ink caret-signal focus:outline-none"
        />
      </div>
