...
import { MARKET_TYPE_LABEL, MOST_LIKELY_TO_MIN_BETTORS, isOptionBased, optionKindLabel, type MarketType } from '@/lib/marketType';
...
function SubjectChips({
  ...
  maxSubjects: number;
  /** Off for the most_likely_to roster picker, where "nobody" isn't an answer: the people picked
   * here are the options themselves, and a market needs at least two of them. */
  nobodyChip?: boolean;
}) {
...
export function CreateMarketForm({
  ...
  // most_likely_to: the members in the running. Each becomes an `@nickname` option on submit, and
  // so a subject of their own option; `members` already excludes the creator, who can never be a
  // subject of their own market (create_market() rejects it), so they can't pick themselves.
  const [picked, setPicked] = useState<MemberOption[]>([]);
  const isMostLikelyTo = marketType === 'most_likely_to';
  const isWhen = marketType === 'when';
  // Preview only: the real four labels are generated by create_market() in the group's timezone
  // at the moment it runs (see lib/whenBuckets.ts). Computed once, since a preview that ticks
  // over to tomorrow mid-form would be more confusing than one that's a minute stale.
  const [whenLabels] = useState(() => whenBucketLabels(new Date(), timezone));
  ...
  // earlier no longer apply. A most_likely_to market is always about the people it names, so the
  // About card never applies to it at all.
  const hasOptionSubject = isMostLikelyTo || (marketType === 'multiple_choice' && options.some((o) => o.label.trim().startsWith('@')));
  useEffect(() => {
    if (hasOptionSubject) setSubjects([]);
  }, [hasOptionSubject]);

  // Soft warning, not a block: the server's only hard rule is the shared subject cap
  // (member_count - 2). Below three the market technically works but reads as a private bet
  // between the creator and the endorser, both of whom count as bettors here since both can bet.
  const maxPicks = Math.min(10, Math.max(0, totalMemberCount - 2));
  const eligibleBettors = totalMemberCount - picked.length;
  const fewBettors = isMostLikelyTo && picked.length > 0 && eligibleBettors < MOST_LIKELY_TO_MIN_BETTORS;
  ...
  function goToStep2() {
    ...
    if (isMostLikelyTo && picked.length < 2) return setError('Pick at least two members to choose between.');
    setStep(2);
  }
  ...
  function submitMarket() {
    ...
    startTransition(async () => {
      const result = await createMarket({
        ...
        // when sends no options at all: create_market() generates its four buckets server-side.
        options:
          marketType === 'multiple_choice'
            ? options.map((o) => o.label.trim())
            : isMostLikelyTo
              ? picked.map((m) => `@${m.nickname}`)
              : undefined,
      });
      if (result.error) {
  ...
  if (step === 1) {
    return (
      <div className="flex flex-1 flex-col gap-[18px]">
            ...
            placeholder={
              marketType === 'multiple_choice'
                ...
                : isMostLikelyTo
                  ...
                  : isWhen
                    ? 'When will Jake finally text her back?'
        ...
        {isMostLikelyTo && (
          <div className={cardClasses}>
            <p className="text-[13px] font-extrabold text-espresso-800">Who&apos;s in the running?</p>
            <SubjectChips members={members} selected={picked} onChange={setPicked} maxSubjects={maxPicks} nobodyChip={false} />
            <p className="mt-2.5 text-[11.5px] leading-[1.45] text-espresso-400">
              Pick 2 to 10 members. Everyone you pick is hidden from this market until it resolves.
            </p>
            {fewBettors && (
              <div className="mt-2.5 flex items-start gap-2.5 rounded-xl bg-honey-50 px-3.5 py-3">
                <InfoIcon className="mt-px h-[17px] w-[17px] shrink-0 text-honey-800" />
                <p className="text-[12.5px] leading-[1.45] text-honey-900">
                  That leaves only {eligibleBettors} people to bet on it, you included. Name fewer members to give it a real
                  pool.
                </p>
              </div>
            )}
        ...
        {isWhen && (
          <div className={cardClasses}>
            ...
            <ol className="mt-3 space-y-2">
              {whenLabels.map((label, i) => (
                <li key={label} className="flex items-center gap-2.5">
                  <span className="w-5 shrink-0 text-center text-sm font-semibold text-espresso-400">{i + 1}</span>
                  <span className="flex-1 rounded-xl border border-espresso-100 bg-espresso-50 px-3.5 py-2.5 text-[15px] text-espresso-700">
                    {label}
                  </span>
                </li>
              ))}
            ...
            <p className="mt-2.5 text-[11.5px] leading-[1.45] text-espresso-400">
              Every When market settles on one of these four. The dates are fixed when the market is created, in group
              time, {friendlyTimezoneName(timezone).replace(/ time$/, '')}.
            </p>
          </div>
        )}
  ...
  return (
    <div className="flex flex-1 flex-col gap-[18px]">
      ...
      <ReviewTicket
        ...
        // A most_likely_to market's hidden-from list is exactly the people picked, so they get the
        // same named chips the About card's subjects do rather than the "every member an option
        // names" summary a typed @option list gets.
        subjects={isMostLikelyTo ? picked : subjects}
        hasOptionSubject={hasOptionSubject && !isMostLikelyTo}
        line={parsedLine}
...
function ReviewTicket({
  ...
  description: string;
  marketType: MarketType;
  /** Option-based types only, already trimmed: typed labels, `@nickname`s, or the four when buckets. Empty otherwise. */
  optionLabels: string[];
  ...
  const kindLabel =
    marketType === 'yes_no' ? 'Yes / No' : marketType === 'over_under' ? 'Over / Under' : optionKindLabel(marketType, optionLabels.length);
  const optionsRowLabel = marketType === 'most_likely_to' ? 'In the running' : marketType === 'when' ? 'Time windows' : 'Options';
  ...
  return (
    <div className="overflow-hidden rounded-[20px] border-[1.5px] border-espresso-800 bg-paper-white">
      ...
      {isOptionBased(marketType) && (
        <ReviewRow label={optionsRowLabel}>
