    ...
    <TicketCard label="You're vouching for" meta={kindLabel} bodyClassName="px-[18px] pt-4 pb-[18px]">
      <div className="space-y-3.5">
        ...
        {options && options.length > 0 && (
          <div>
            <p className="text-[11.5px] font-extrabold tracking-[0.08em] text-faint uppercase">The options</p>
            <ul className="mt-1.5 flex flex-col gap-1.5">
              {options.map((o) => (
                <li
                  key={o.id}
                  className="rounded-[14px] border-[1.5px] border-hairline bg-canvas px-3.5 py-2.5 text-sm font-bold text-muted"
                >
...
function VouchRow({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="flex items-center justify-between gap-3">
      <span className="text-[11px] font-extrabold tracking-[0.1em] text-faint uppercase">{label}</span>
      <span className="min-w-0 truncate text-sm font-bold text-ink">{children}</span>
    </div>
  );
}
