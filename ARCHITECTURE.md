...
- **`expire_stale()`** — the cron entry point, scheduled via pg_cron every minute (`barbets-expire-stale`, same cadence as send-push below — tightened from an original 5-minute interval once send-push proved 1-minute pg_cron scheduling works fine on this project's plan). Covers all four timers: 72h unsponsored expiry, market auto-close at `closes_at` (stamping `closed_at`), 8h no-challenge auto-finalize, 8h vote-window auto-finalize, and timed season auto-end. `get_cron_job_status()` (service_role only) reports both jobs' registered schedules for debugging, since `cron.job` isn't exposed via PostgREST directly.
