...
- **`expire_stale()`** — the cron entry point. Covers all four timers: 72h unsponsored expiry, market auto-close at `closes_at` (stamping `closed_at`), 8h no-challenge auto-finalize, 8h vote-window auto-finalize, and timed season auto-end.
