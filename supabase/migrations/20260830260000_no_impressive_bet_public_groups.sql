-- _finalize_market_core: identical to 20260825110000_public_group_instant_resolution.sql's version
-- except the closing "impressive bet" push is now skipped for a public group. A directory-joined
-- public group can run into the hundreds of members betting on dozens of auto-generated Sports/
-- Weather markets a day -- "biggest underdog win in the group's history" reads as a real, rare
-- milestone in a small group of friends, but turns into routine noise once the group is that large
-- and that active. v_is_public is already computed above for the challenge-window skip, so this is
-- a pure read of an existing local, not a new lookup.
