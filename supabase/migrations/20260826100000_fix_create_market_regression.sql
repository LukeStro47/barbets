-- Fixes a real regression: 20260824120000_public_group_market_gates.sql was built on a stale base
-- (20260806150000_fix_create_market_overload.sql) that predates two later fixes to create_market —
-- 20260810170000_name_and_unit_length_limits.sql (unit cap tightened from 12 to 10 characters, plus
-- market title blank/140-char validation) and 20260817120000_option_label_length_limit.sql (the
-- 40-char option-label cap) — and silently reverted both while adding the public-group gates.
-- Caught by tests/integration/markets.multiple_choice.test.ts's "an option label over 40 characters
-- is rejected" failing in CI. This redeclares create_market from the true latest base
-- (20260817120000) with the same public-group additions re-applied correctly on top: same 9-param
