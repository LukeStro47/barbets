-- 20260828130000_public_group_pipeline_notifications.sql removed _create_system_market()'s own
-- perform _emit_notification_event('market_opened', ...) call, moving that decision entirely to
-- _notify_system_markets_created() (one named push for a single-market run, one consolidated
-- system_markets_opened push for a multi-market run) so a six-market Weather run fires one push
-- instead of six. 20260830300000_system_market_multiple_choice.sql then redeclared this function
-- from a base that predated that removal -- same "redeclared from a stale base" failure
-- ARCHITECTURE.md already tracks for create_market() and update_group_settings(), this time on
-- _create_system_market() -- silently reinstating the direct emit. 20260831100000 (the
-- is_system_market flag fix) copied that stale body forward again without noticing. Every
-- Sports/Weather market created since 20260830300000 landed has been double-pushed: once here,
-- once more from _notify_system_markets_created()'s own named or consolidated event.
--
-- Fix: same 8-arg signature as 20260831100000, so a plain CREATE OR REPLACE is correct per this
-- project's function-change rule -- this migration changes exactly one thing, deleting the
-- trailing emit call, and is otherwise byte-for-byte 20260831100000's body.
