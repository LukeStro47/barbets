-- update_group_settings gains a trailing p_awards_enabled param, same pattern as every other
-- plain settings flag (betting_enabled, accepting_members, allow_hedged_bets, require_endorsement)
-- — freely reversible in either direction, since turning it off just stops _recompute_group_titles/
-- _upsert_risk_taker from writing (see 20260824130000_awards_enabled_gate.sql) rather than
-- corrupting any in-flight state. Per ARCHITECTURE.md's documented overload gotcha, the current
-- 14-arg signature (from 20260822130000_group_join_message.sql) is dropped explicitly rather than
-- left for CREATE OR REPLACE to orphan alongside a new 15-arg overload.
drop function if exists update_group_settings(uuid, int, boolean, season_length, text, boolean, boolean, boolean, int, boolean, timestamptz, numeric, boolean, text);
...
begin
  ...
  -- "No awards" is one of a public group's four fixed rules, not a per-group preference — force it
  -- off regardless of what's submitted rather than letting a stray form post re-enable it.
