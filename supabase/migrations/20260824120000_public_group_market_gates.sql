-- Two of the four public-group rules ("only mods can create markets" and "no markets about a
-- specific person") live in create_market() itself, right next to the membership/subject-cap
-- checks they sit alongside. Same 9-param signature as 20260806150000_fix_create_market_overload.sql
-- (p_unit trailing), so this is a plain CREATE OR REPLACE — nothing about the signature changed,
-- only body logic gated on groups.is_public.
