-- propose_resolution: identical to 20260825110000_public_group_instant_resolution.sql's version
-- except a public group's market no longer emits resolution_proposed at all. That event's own copy
-- ("You have {resolution_window_hours} to challenge it") is simply false for a public group -- there
-- is no challenge window, propose_resolution finalizes in the very same transaction a few lines
-- below -- and a bettor who actually gets notified (get_event_recipients already restricts this
-- event, in a public group, to people who bet on the market) would get it immediately followed by
-- the real market_resolved push seconds later. That's not a heads-up, it's noise right before the
-- answer, same reasoning the Weather pipeline's dropped market_closed push already uses. A private
-- group's market is untouched: it still gets resolution_proposed exactly as before, since a real
-- challenge window follows it there.
