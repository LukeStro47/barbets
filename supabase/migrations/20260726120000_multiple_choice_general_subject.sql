-- Multiple choice markets could only ever be "about" someone via a leading
-- @nickname on one of the options themselves. That leaves no way to hide a
-- market from someone when none of the options happen to name a person
-- (e.g. "What will Jake wear to the wedding?" with options "Tux"/"Suit"/
-- "Hawaiian shirt" — the market is about Jake, but no option is). This lets
-- create_market() take p_subject_user_ids for multiple_choice too, the same
-- market-level (option_id null) mechanism yes_no/over_under already uses —
-- but only when no option already carries its own @mention, so there's
-- never two different, possibly-overlapping sources of "who is this hidden
-- from." The client only ever offers one or the other; this validation is
-- the server-side backstop. Same signature as before, so no overload to
-- drop.
