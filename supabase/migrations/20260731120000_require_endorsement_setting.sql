-- Endorsement (a second member sponsoring a market before it opens) was a
-- hardcoded requirement for every group. Makes it an owner-configurable
-- group_settings toggle instead, default on so every existing group's
-- behavior is unchanged: `create_market()` now opens a market directly
-- (status = 'open', no sponsor_id, market_opened notification) instead of
-- routing it through pending_sponsor when the group has turned this off.
-- Good for a small, high-trust group where waiting on a second person to
-- endorse every market is friction with no real benefit, since nobody's
-- worried about someone quietly starting a bad-faith market solo.
--
-- Deliberately NOT touching the subject-count cap (member_count - 2,
-- reserving a slot each for the creator and endorser — see ARCHITECTURE.md).
-- A require_endorsement=false market never actually needs that reserved
-- endorser slot, so the cap is technically one subject more conservative
-- than strictly necessary for such a group, but relaxing a privacy/money
-- -adjacent invariant isn't worth doing as a side effect of this setting.
-- Revisit if a group with require_endorsement off actually runs into the cap.
alter table group_settings add column require_endorsement boolean not null default true;

-- update_group_settings: new trailing p_require_endorsement param. Per
-- ARCHITECTURE.md's documented overload gotcha, the old 13-arg signature is
-- dropped explicitly rather than left for CREATE OR REPLACE to orphan.
drop function if exists update_group_settings(uuid, int, boolean, season_length, text, boolean, boolean, boolean, int, int, boolean, timestamptz, numeric);
...
revoke execute on function update_group_settings(uuid, int, boolean, season_length, text, boolean, boolean, boolean, int, int, boolean, timestamptz, numeric, boolean) from public;
grant execute on function update_group_settings(uuid, int, boolean, season_length, text, boolean, boolean, boolean, int, int, boolean, timestamptz, numeric, boolean) to authenticated;

-- create_market(): same signature as before (no overload to drop), only the
-- body changes. Skips pending_sponsor/sponsor_id entirely and opens directly
-- when the group has turned endorsement off; the subject-cap error text
-- (unchanged cap number, see the comment at the top of this migration) now
-- names the actual roles a market in this group needs bodies for.
