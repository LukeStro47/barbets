-- Multiple-choice option labels had no length cap anywhere: not on the create-market
-- input, not in create_market's own validation. A long option ("Sad drunk sitting in
-- corner", or worse) renders fine in the option list, but every other place an option
-- label shows up assumes it's short: the "On" value on the bet-confirmed ticket sits
-- right next to the staked amount at 38px with no guaranteed split between them, and
-- the trailing bet pill on a market row is allowed to grow instead of the title next
-- to it. 40 matches the client's new maxLength (lib/limits.ts OPTION_LABEL_MAX_LENGTH)
-- and comfortably fits every real option already in use.
--
-- Redeclared verbatim from 20260810170000_name_and_unit_length_limits.sql with only
-- the option-length check added inside the multiple_choice branch's existing
-- per-option loop. Same 9-parameter signature, so this is a true replace, not a new
-- overload.
