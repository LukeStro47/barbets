-- New groups start with betting off — the owner has to consciously flip a
-- switch in settings before anyone can create a market. Defaults to false at
-- the column level, so create_group()'s existing INSERT (which doesn't list
-- this column) picks it up automatically with no signature change needed.
alter table group_settings add column betting_enabled boolean not null default false;
...
-- Also fixes a latent bug from the earlier time-zone migration: adding
-- p_timezone as a new trailing default never dropped the original 5-param
-- signature, leaving a second overload sitting in the catalog undetected
-- (nothing ever called it without p_timezone, so PostgREST's ambiguity
-- error never surfaced). Dropping both stale signatures here so only the
-- final 7-param version remains.
drop function if exists update_group_settings(uuid, int, int, boolean, season_length);
drop function if exists update_group_settings(uuid, int, int, boolean, season_length, text);
...
revoke execute on function update_group_settings(uuid, int, int, boolean, season_length, text, boolean) from public;
grant execute on function update_group_settings(uuid, int, int, boolean, season_length, text, boolean) to authenticated;
