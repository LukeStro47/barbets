-- Same validation as before (subjects must be fewer than group members - 2,
-- so a creator, an endorser, and at least one bettor can always exist
-- outside the subject set) — the only change is an error message that says
-- the actual numbers instead of just "too many subjects", since the
-- silent-feeling rejection was confusing in a small test group.
