-- Reverses the "pipeline-only board" rule from 20260828130000/20260829170000: Sports and Weather
-- moderators (and the owner) can now hand-create a market there too, same as any other public
-- group's mod-or-owner check just below. This is purely additive to what the pipeline creates on
-- its own -- sports-create-markets/weather-create-markets cap their own output at 3 *open system*
-- markets per group by counting `is_system_market = true` rows specifically (see each function's
-- `openCount` query), and a hand-created market never sets that column, so it can never eat into
-- or be counted against the pipeline's cap either way.
--
-- Same 9-param signature as 20260829170000, plain CREATE OR REPLACE, no drop needed. Only the
-- Sports/Weather name-based block is removed; every other check (mod-or-owner, no @mentions/
-- subjects, everything below) is untouched.
