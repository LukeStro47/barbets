-- Two product changes from real usage:
-- 1. The vote window shrinks from 48h to 24h, matching the challenge
--    window, so both post-close timers are the same length.
-- 2. finalize_market() now excludes whoever triggered it from the
--    'market_resolved' notification — most visibly, the last voter whose
--    ballot completed turnout and auto-finalized the market no longer gets
--    pushed a notification about the thing they just did. This falls out
--    for free from using auth.uid(): when cast_vote()'s auto-finalize path
--    calls finalize_market() in the same request/transaction, auth.uid()
--    is still that voter; when expire_stale() (cron) calls it, there's no
--    request context at all, so auth.uid() is null and nobody is excluded.
--    A manual "Finalize now" click gets the same treatment as a side
--    effect, which is the right call for the same reason.
...
-- expire_stale()'s disputed sweep window matches the new 24h vote window
-- (was 48h) so cron picks up a stale disputed market at the same boundary
-- finalize_market() itself now enforces.
