-- Records exactly where a universal-loss market's pool went when
-- distribute_payout fires, so the reveal page can show it. This can't be
-- reconstructed after the fact from the ledger alone: the "split across
-- other markets" amount is a bonus_pool transfer, not a balance change, so
-- it never gets a ledger row (see the distribute_payout migrations) — and
-- even the ledger rows that do exist (creator/endorser cuts) aren't tagged
-- with *why*, just a generic 'payout' reason. Null for every other
-- finalize_market() path (a normal win, a VOID, or distribute_payout off).
alter table markets add column payout_breakdown jsonb;
