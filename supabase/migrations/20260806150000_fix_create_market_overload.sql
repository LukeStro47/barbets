-- Fixes a mistake in the previous migration: create_market was redeclared
-- using an outdated 8-parameter signature (missing p_unit, added by
-- 20260720100000_over_under_units.sql, and several other changes added
-- since — general multiple_choice subjects, require_endorsement routing,
-- pending_bonus_pool, market_opened_about_you). Postgres identity is the
-- argument type signature, not defaults, so that created a second overload
-- alongside the real one instead of replacing it (the exact "bitten four
-- times already" trap ARCHITECTURE.md documents) — confirmed by
-- createMarket() calls immediately failing with "Could not choose the best
-- candidate function." Drop the stray 8-param overload, and redeclare the
-- real 9-param signature (verbatim from 20260805140500_market_opened_about_you.sql)
-- with only the intended status exclusions added.
drop function if exists create_market(uuid, text, text, market_type, timestamptz, numeric, uuid[], text[]);
