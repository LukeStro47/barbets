-- 20260903100000 redeclared update_group_settings() from a stale base that predated both
-- 20260830240000 (added p_prize_text/p_punishment_text) and 20260901180000 (added the
-- settings_update audit logging), so its CREATE OR REPLACE used the old 15-arg signature
-- instead of the current 17-arg one. Since a function's identity is its argument type list,
-- that didn't replace the real function -- it created a second overload sitting alongside it,
-- and PostgREST can no longer pick a candidate for any call, breaking every caller. This is the
-- same "redeclared from a stale base" failure ARCHITECTURE.md already documents for
-- create_market(), now a fourth instance, on update_group_settings() specifically this time.
--
-- Fix: drop the stale 15-arg overload, and recreate the function on the 17-arg signature,
-- carrying forward everything that landed since 20260901180000 (prize/punishment fields,
-- settings_update audit logging) plus the season_id backfill 20260903100000 was actually
-- trying to add.
