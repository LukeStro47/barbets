-- create_market: for a seasons-enabled group, the betting gate moves from
-- the group-level (one-way) group_settings.betting_enabled to the current
-- season's own betting_open flag — every season starts paused, and the
-- owner opens it per-season via open_season_betting(). Continuous
-- (non-seasons) groups are unaffected, still gated by betting_enabled
-- exactly as before. Also adds a closes_at upper bound: a market can't be
-- created to close later than its season's frozen ends_at (manual-length
-- seasons have no ends_at, so no bound applies).
