-- Turning seasons_enabled on for a group that was already taking bets continuously (20260830140000
-- made season 1 inherit betting_open so that group stays live) created season 1 but never touched
-- the group's existing markets: they keep season_id = null forever, since create_market() only
-- stamps season_id when seasons_enabled is already true at creation time. Every season-scoped read
-- added since then (the leaderboard's "nobody's bet yet" bet count, get_head_to_head_markets, etc.)
-- then can't see that pre-switch history at all, and a group with real, resolved bets shows an empty
-- leaderboard because none of its markets carry the new season 1's id.
--
-- Fix going forward: update_group_settings() backfills season_id onto every season_id-null market
-- in the group the moment season 1 is created, attributing the group's whole pre-switch history to
-- that first season rather than leaving it orphaned. Same 15-arg signature, plain CREATE OR REPLACE.
--
-- Fix for groups already affected: a market can only have season_id = null while its group has any
-- seasons row at all in exactly this scenario (once seasons_enabled is true, create_market() refuses
-- to run without an active season to stamp), so backfilling every such market to its group's season
-- 1 is safe and unambiguous.
update markets
set season_id = s.id
from seasons s
where markets.season_id is null
  and markets.group_id = s.group_id
  and s.number = 1;
