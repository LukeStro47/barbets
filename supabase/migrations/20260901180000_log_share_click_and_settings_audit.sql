-- 5b: share button click logging. Unlike log_qr_scan() (anon-callable, fires before
-- any session exists), this only ever fires from inside the authenticated app, so
-- it's authenticated-only like an ordinary mutation function. The two call sites
-- (RevealTicket.tsx, ShareRecordCard.tsx) stay behind SHARE_BUTTONS_ENABLED, which
-- stays false -- this just makes the instrumentation ready for whenever that flips,
-- with no visible behavior change today.
create function log_share_click(p_context text, p_group_id uuid default null)
returns void
language plpgsql
security definer
...
begin
  ...
  values ('share_click', auth.uid(), p_group_id, jsonb_build_object('context', left(p_context, 32)));
end;
$$;

revoke execute on function log_share_click(text, uuid) from public;
grant execute on function log_share_click(text, uuid) to authenticated;

-- 5d: update_group_settings() gets a diff computed against the pre-update row
-- (v_settings, already selected at this point in the function) so the admin panel
-- can see how often settings change and whether the change touched "advanced"
-- fields (require_endorsement, allow_hedged_bets, distribute_payout,
-- creator_payout_pct, resolution_window_hours, prize_text, punishment_text -- the
-- set behind the group-creation wizard's "Advanced settings" label) versus "basic"
-- ones (everything else this function writes). The diff is computed from the
-- POST-coercion values -- a public group's forced overrides earlier in this function
-- would otherwise misread as the caller having changed 7 advanced fields on every
-- single save. basic_changed and advanced_changed are not mutually exclusive: one
-- save can touch both tiers, and both flags get set true in that case. A resubmit
-- with no actual changes (the settings form resubmits every field on every save)
-- inserts no event at all. Same 17-arg signature as before, plain create or replace,
-- no drop needed.
...
begin
  ...
  v_basic_changed := v_changed_fields && array['seed_amount', 'seasons_enabled', 'season_length', 'timezone', 'accepting_members', 'betting_enabled', 'season_custom_ends_at', 'join_message', 'awards_enabled'];
