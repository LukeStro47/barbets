-- Pure re-push, no logic change: staging's live create_market() was found to be missing the
-- Sports/Weather hand-create gate that 20260828130000_public_group_pipeline_notifications.sql
-- added, even though that migration is merged to main and its version is recorded as applied.
-- Reproduced directly against staging: creating a market in an is_public group literally named
-- 'Sports' succeeded instead of raising invalid_operation. tests/integration/public_groups.test.ts
-- ("Sports/Weather are pipeline-only boards") caught it in CI.
--
-- The other functions from that same migration (system_markets_opened consolidation, Weather's
-- market_closed skip in expire_stale) are still behaving correctly on staging, which rules out
-- the whole migration silently no-op'ing — only this one statement's effect was lost. The likely
-- mechanism is exactly what this repo's CI comments already warn about: staging is one shared,
-- persistent database, and --include-all lets any open PR's migrations land there out of
-- timestamp order. A PR branched before 20260828130000 existed, carrying its own unrelated
-- CREATE OR REPLACE of create_market from a pre-gate base, would silently regress this the moment
-- its CI pushed to staging - migration tracking is by version/filename, not by function body
-- content, so nothing catches that.
--
-- Same remedy as 20260826100000_fix_create_market_regression.sql used for an equivalent
-- staging/repo mismatch: redeclare from the true latest base (20260828130000) unchanged, under a
-- new version, so this PR's CI reasserts the correct function on staging regardless of what most
-- recently clobbered it. Identical body, same 9-param signature, plain CREATE OR REPLACE.
