-- Fixes a real regression I introduced in 20260829130000_lifecycle_event_instrumentation.sql:
-- that migration redeclared create_market() from a stale base (20260826100000_fix_create_market_regression.sql),
-- which predates 20260828130000_public_group_pipeline_notifications.sql's Sports/Weather
-- hand-create gate (`if v_is_public and v_group_name in ('Sports', 'Weather') then raise
-- exception...`). Pushing that stale-based redeclare silently removed the gate from production --
-- caught by tests/integration/public_groups.test.ts's "Sports/Weather are pipeline-only boards"
-- tests failing after the fact, not by review.
--
-- This redeclares create_market() from the true latest base (20260828130000) with the gate
-- restored, plus the same market_create lifecycle_events insert 20260829130000 already added.
-- Same 9-param signature, plain CREATE OR REPLACE, no drop needed.
